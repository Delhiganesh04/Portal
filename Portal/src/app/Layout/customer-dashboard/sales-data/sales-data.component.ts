...
export class SalesDataComponent implements OnInit {
  ...
  applyFilters(): void {
    this.filteredOrders = this.salesOrders.filter(order => {
      ...
      // Filter by PO number
      const matchesPONumber = !this.poNumber || 
        (order.Bstnk && order.Bstnk.toLowerCase().includes(this.poNumber.toLowerCase()));
      ...
      return matchesSearch && matchesStatus && matchesDocType && 
             matchesDeliveryStatus && matchesPONumber && matchesDateRange;
    });
  }
